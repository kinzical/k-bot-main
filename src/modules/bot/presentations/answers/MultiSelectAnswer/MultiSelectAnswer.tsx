...
type MultiSelectAnswerProps = {
  data: ActivityVM;
};

const MultiSelectAnswer: React.FC<MultiSelectAnswerProps> = (
  props: MultiSelectAnswerProps
) => {
  const { data } = props;
  const { answer } = data;
  const text = answer?.options?.map(
    (item: any, index: number) =>
      item.label + (index !== answer.options.length - 1 ? ", " : "")
  );
